# 2026-03-22 | 숨고 스피치 컨설팅 견적 요청 플로우 이벤트 QA 기록 (풀텍스트)
...
하지만 실제 서비스의 내부 스키마 문서, Amplitude 프로젝트 설정, warehouse 테이블에는 접근할 수 없었습니다.

그래서 이번 프로젝트에서는 접근 가능한 관찰 수단인 **Amplitude Event Explore**를 활용해,
...
이 프로젝트의 초점은 **도구 사용 자체**가 아니라,
**제한된 관찰 환경에서 로그 QA 관점으로 이벤트 수집 상태를 읽어내는 과정**에 있습니다.
...
여기서 중요한 건 “요청폼 step이 몇 개인가”를 단정하는 것이 아니라,
**실제 raw에서 step 이벤트가 어떤 구조로 반복해서 쌓이는가**를 보는 것이었습니다.
